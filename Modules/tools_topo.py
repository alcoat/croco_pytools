...
####
    jmin=tools.indx_bound(topo_lat, geolim[2])
    jmax=tools.indx_bound(topo_lat, geolim[-1])
    ...
    imin=tools.indx_bound(topo_lon, geolim[0])
    imax=tools.indx_bound(topo_lon, geolim[1])
    ...
    if 0 < imin and imin < topo_lon.shape[0] and 0 < imax and imax < topo_lon.shape[0] :
        ...
        if imin==0 :
            shft_west=-1
            imin=tools.indx_bound(topo_lon, geolim[0]+360)
        ...
        if imax == 0:
            shft_east=-1
            imax=tools.indx_bound(topo_lon, geolim[1]+360)
