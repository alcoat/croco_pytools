...
class getdata():
    def __init__(self,inputdata,inputfile,crocogrd,multi_files,file_format,tideslist,current=None):
        ...
        if current is not None: 
            ...
            if inputdata=='tpxo' and self.ncglo['u_part1'].units=='centimeter^2/sec':
                print('TPXO is in format %s for velocity unit. Looking for a grid file to change that.' % self.ncglo['u_part1'].attrs['units'])
                grd_file=glob.glob(current[0].replace(current[0].split('/')[-1],'grid*'))
                if len(grd_file)>=1:
                    print('Found '+grd_file[0]+' for grid file')
                    xar=xr.open_dataset(grd_file[0])
                    hu=xar.hu*100
                    hv=xar.hv*100
                    self.ncglo['u_part1']=self.ncglo['u_part1']/hu
                    self.ncglo['u_part1'].attrs['units']='centimeter/sec'
                    self.ncglo['u_part2']=self.ncglo['u_part2']/hu
                    self.ncglo['u_part2'].attrs['units']='centimeter/sec'
                    self.ncglo['v_part1']=self.ncglo['v_part1']/hv
                    self.ncglo['v_part1'].attrs['units']='centimeter/sec'
                    self.ncglo['v_part2']=self.ncglo['v_part2']/hv
                    self.ncglo['v_part2'].attrs['units']='centimeter/sec'                    
                else:
                    sys.exit('Did not find TPXO grid file, exit...')
        ...
        for key in self.ncglo:
            if 'u' in self.ncglo[key]:
                ...
                ny=eval(''.join(("dataxr_uv."+self.var['latu']))).shape[0]
                ...
                ny=eval(''.join(("dataxr_uv."+self.var['latv']))).shape[0]
                if (self.ncglo[key][:].shape[1] != ny and self.ncglo[key][:].shape[2] != nx ) and (self.ncglo[key][:].shape[1] == nx and self.ncglo[key][:].shape[2] == ny):
                    print('%s in format [ntides,Lon,Lat], switching Lon/Lat axes' % key)
                    dim=self.ncglo[key].dims
                    self.ncglo[key]=self.ncglo[key].transpose(dim[0],dim[2],dim[1])
                ...
                ny=eval(''.join(("dataxr_ssh."+self.var['latr']))).shape[0]
                if (self.ncglo[key][:].shape[1] != ny and self.ncglo[key][:].shape[2] != nx ) and (self.ncglo[key][:].shape[1] == nx and self.ncglo[key][:].shape[2] == ny):
                    print('%s in format [ntides,Lon,Lat], switching Lon/Lat axes' % key)
                    dim=self.ncglo[key].dims
                    self.ncglo[key]=self.ncglo[key].transpose(dim[0],dim[2],dim[1])       
...
#            N=N+360 
        ...
        ##
        if input_wav.lower() in ['sa','ssa','msf','alpha1','tau1','pi1','p1','s1','psi1','phi1','theta1','m2a','m2b','lambda2','t2','s2','r2','s3','s4','s5','s6','s7','s8']:
            pf = 1.
            ...
            pf = sqrt(temp1**2 + temp2**2)    
        else:
            print('No amplitude correction for wave %s' % input_wav)
            pf = 0
        
        ##
        if input_wav.lower() in ['sa','ssa','mm','msf','alpha1','tau1','pi1','p1','s1','psi1','phi1','theta1','m2a','m2b','lambda2','t2','s2','r2','s3','s4','s5','s6','s7','s8']:
            pu=0
        elif input_wav.lower() in ['2q1','sigma1','q1','rho1']:
