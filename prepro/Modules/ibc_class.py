...
class getdata:
    def __init__(
        self, inputdata, inputfile, crocogrd, multi_files, tracers=[], bdy=None
    ):  # bdy=[obs,tstart,tend,cycle]
        ...
        if bdy is None:  # Ini case
            ...
            [
                ...
                self.jdmax,
                self.period,
            ] = self.handle_periodicity(crocogrd, "r")
            [
                self.lonU,
                self.latU,
                self.idminU,
                ...
                self.jdmaxU,
                self.periodU,
            ] = self.handle_periodicity(crocogrd, "u")
            [
                self.lonV,
                self.latV,
                self.idminV,
                ...
                self.jdmaxV,
                self.periodV,
            ] = self.handle_periodicity(crocogrd, "v")
        elif bdy is not None and bdy[-1] == 0:  # bdy case
            if multi_files == False:
                dataxr = xr.open_mfdataset(
                    inputfile, combine="nested", concat_dim=self.var["time_dim"]
                )
                self.depth = dataxr[self.var["depth"]]
                dataxr = xr.open_mfdataset(
                ...
                for trc in tracers:
                    self.ncglo[trc] = xr.open_mfdataset(
                        inputfile[trc],
                        combine="nested",
                        concat_dim=self.var["time_dim"],
                    )
            ...
            for boundary, is_open in zip(bdy[0].keys(), bdy[0].values()):
                ...
                if "west" in boundary and is_open:
                    ...
                    [
                        ...
                        self.jdmaxW,
                        self.periodW,
                    ] = self.handle_periodicity(crocogrd, "r", bdy="west")
                    [
                        self.lonUW,
                        self.latUW,
                        self.idminUW,
                        ...
                        self.jdmaxUW,
                        self.periodUW,
                    ] = self.handle_periodicity(crocogrd, "u", bdy="west")
                    [
                        self.lonVW,
                        self.latVW,
                        self.idminVW,
                        ...
                        self.jdmaxVW,
                        self.periodVW,
                    ] = self.handle_periodicity(crocogrd, "v", bdy="west")
                    ...
                    [
                        ...
                        self.jdmaxE,
                        self.periodE,
                    ] = self.handle_periodicity(crocogrd, "r", bdy="east")
                    [
                        self.lonUE,
                        self.latUE,
                        self.idminUE,
                        ...
                        self.jdmaxUE,
                        self.periodUE,
                    ] = self.handle_periodicity(crocogrd, "u", bdy="east")
                    [
                        self.lonVE,
                        self.latVE,
                        self.idminVE,
                        ...
                        self.jdmaxVE,
                        self.periodVE,
                    ] = self.handle_periodicity(crocogrd, "v", bdy="east")
                    ...
                    [
                        ...
                        self.jdmaxS,
                        self.periodS,
                    ] = self.handle_periodicity(crocogrd, "r", bdy="south")
                    [
                        self.lonUS,
                        self.latUS,
                        self.idminUS,
                        ...
                        self.jdmaxUS,
                        self.periodUS,
                    ] = self.handle_periodicity(crocogrd, "u", bdy="south")
                    [
                        self.lonVS,
                        self.latVS,
                        self.idminVS,
                        ...
                        self.jdmaxVS,
                        self.periodVS,
                    ] = self.handle_periodicity(crocogrd, "v", bdy="south")
                    ...
                    [
                        ...
                        self.jdmaxN,
                        self.periodN,
                    ] = self.handle_periodicity(crocogrd, "r", bdy="north")
                    [
                        self.lonUN,
                        self.latUN,
                        self.idminUN,
                        ...
                        self.jdmaxUN,
                        self.periodUN,
                    ] = self.handle_periodicity(crocogrd, "u", bdy="north")
                    [
                        self.lonVN,
                        self.latVN,
                        self.idminVN,
    ...
    def handle_periodicity(self, crocogrd, grid, bdy=None):
        ...
        if bdy is not None:
            geolim = [
                ...
                np.max(getattr(crocogrd, "lon_" + bdy)) + buffer_zone,
                np.min(getattr(crocogrd, "lat_" + bdy)) - buffer_zone,
                np.max(getattr(crocogrd, "lat_" + bdy)) + buffer_zone,
            ]
        else:
            geolim = [
        ...
        if -1 < imin and imin < lon.shape[0] and -1 < imax and imax < lon.shape[0]:
            ...
            period = 0
            print(
                "Single region dataset imin/imax=",
                imin,
                imax,
            )
        else:
            ...
            if -1 < imin and imin < lon.shape[0] and -1 < imax and imax < lon.shape[0]:
                ...
                print(
                    "ERROR: Data longitude covers 360 degrees,"
                    " but still cannot find  starting and ending indices."
                )
                sys.exit(1)
        ...
        ###
            ...
            print(
                "Reading topography in two separate parts"
                " adjacent through 360-degree periodicity\n First..."
            )
