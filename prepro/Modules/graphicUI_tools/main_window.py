...
class Inputs_c2c(HasTraits):
    ...
    coef = CInt(3, desc="Refinement coefficient", label="Refinement coef")
...
class MainWindowHandler(Handler):
    def close(self, info, is_OK):
        if (
            info.object.panel.compute_grid_thread
...
class MainWindow(HasTraits):
    ...
    panel = Group(
        ...
        Group(
            ...
            Item(
                name="opt_file",
                editor=DirectoryEditor(entries=1),
                style="simple",
                show_label=True,
            ),
            Item(name="save_grid", show_label=False),
            label="Topo smoothing",
            dock="tab",
        ),
        ########## Third tab ######
        Group(
            ...
            Item(
                ...
                editor=CheckListEditor(
                    values=["South", "West", "East", "North"], cols=4
                ),
                style="custom",
            ),
            Item(
                name="merge",
                label="Merging area (nb points)",
                style="simple",
                springy=True,
            ),
            #                          '_',
    ...
    view = View(
        HSplit(
            Item("figure", editor=MPLFigureEditor(), dock="vertical", show_label=False),
            panel,
        ),
        resizable=True,
        height=0.90,
    ...
    def _compute_smooth_fired(self):
        ...
        self.compute_smooth_thread.single_connect = [
            self.single_connect,
            self.sglc_i,
            self.sglc_j,
        ]
    ...
    def _compute_zm_fired(self):
        ...
        self.compute_zm_thread.single_connect = [
            self.single_connect,
            self.sglc_i,
            self.sglc_j,
        ]
    ...
    def _compute_c2c_fired(self):
        ...
        self.compute_c2c_thread.single_connect = [
            self.single_connect,
            self.sglc_i,
            self.sglc_j,
        ]
    ...
    def _save_grid_c2c_fired(self):
        ...
        self.save_grid_c2c_thread.prt_grd = [
            True,
            self.croco_file,
            self.inputs_c2c.coef,
            self.inputs_c2c.imin,
            self.inputs_c2c.imax,
            self.inputs_c2c.jmin,
            self.inputs_c2c.jmax,
        ]
    ...
    def grid_show(self, smooth=None):
        ...
        def outline(lon, lat):
            ...
            def func(var):
                return np.hstack(
                    [var[:, 0], var[-1, 1:-1], var[::-1, -1], var[0, ::-1][1:]]
                )
        ...
        ox_psi, oy_psi = outline(
            self.outputs.lon_psi[1:-1, 1:-1], self.outputs.lat_psi[1:-1, 1:-1]
        )
        ...
        if self.inputs.proj == "Plate":
            ax = self.figure.add_axes(
                rect=[0.1, 0.04, 0.80, 0.9],
                projection=ccrs.PlateCarree(central_longitude=mid),
            )
        elif self.inputs.proj == "North Pole":
            ax = self.figure.add_axes(
                rect=[0.1, 0.04, 0.80, 0.9], projection=ccrs.NorthPolarStereo()
            )
        elif self.inputs.proj == "South Pole":
            ax = self.figure.add_axes(
                rect=[0.1, 0.04, 0.80, 0.9], projection=ccrs.SouthPolarStereo()
            )
        ...
        if smooth is None:
            ...
            if not "mask" in self.inputs.zview:
                ...
                ax.add_feature(
                    shape_feature, facecolor="#929591", edgecolor="Gainsboro", zorder=4
                )
                gl = ax.gridlines(
                    draw_labels=True,
                    linewidth=2,
            ...
            ax.pcolormesh(
                x_rho,
                y_rho,
                np.ma.masked_where(self.outputs.mask_rho > 0, self.outputs.mask_rho),
                vmin=0,
                vmax=1,
                zorder=2,
                cmap=matplotlib.pyplot.cm.copper_r,
                transform=ccrs.PlateCarree(),
            )
            ...
            cb = ax.pcolormesh(
                x_rho,
                y_rho,
                np.ma.masked_where(self.outputs.mask_rho < 1, self.outputs.h),
                zorder=2,
                transform=ccrs.PlateCarree(),
            )
            pos_cax = self.figure.add_axes([pos_x, pos_y, cax_width, cax_height])
    ...
    def grid_show_zm(self, prt_grd):
        ...
        prt_oxp, prt_oyp = outline(
            prt_grd.lon_psi[1:-1, 1:-1], prt_grd.lat_psi[1:-1, 1:-1]
        )
        chd_oxp, chd_oyp = outline(
            self.outputs.lon_psi[1:-1, 1:-1], self.outputs.lat_psi[1:-1, 1:-1]
        )
        ...
        ax.plot(
            prt_oxp, prt_oyp, "r", zorder=5, transform=ccrs.PlateCarree()
        )  # , ax=self.figure.gca(), zorder=3)
        ...
        ax.pcolormesh(
            prt_xr,
            prt_yr,
            np.ma.masked_where(prt_grd.mask_rho > 0, prt_grd.mask_rho),
            vmin=0,
            vmax=1,
            zorder=2,
            cmap=matplotlib.pyplot.cm.copper_r,
        ...
        ax.pcolormesh(
            prt_xr,
            prt_yr,
            np.ma.masked_where(prt_grd.mask_rho < 1, prt_grd.h),
            vmin=np.nanmin(prt_grd.h),
            vmax=np.nanmax(prt_grd.h),
            zorder=3,
            transform=ccrs.PlateCarree(),
        )
