...
class getdata:
    def __init__(
        ...
        self.var = dico.lookvar(
            inputdata
        )  # Dictionary to find the names of the input variables
        if file_format == "Amp_Phase":
            ...
            sys.exit(
                "input_type must be Amp_Phase or Re_Im depending on what is in your input file(s)"
            )
        ...
        if currentu is not None:
            ...
            if "tpxo" in inputdata and "transport" in self.ncglo["u_part1"].long_name:
                ...
                grd_file = glob.glob(
                    currentu[0].replace(currentu[0].split("/")[-1], "grid*")
                )
                # check transport units to change to m2/s
                if "units" not in self.ncglo["u_part1"].attrs or self.ncglo[
                    "u_part1"
                ].attrs["units"] in ["cm", "centimeter"]:
                    coef = 100
                elif (
                    "mm" in self.ncglo["u_part1"].attrs["units"]
                    or "millimeter" in self.ncglo["u_part1"].attrs["units"]
                ):
                    coef = 1000
                else:
                    coef = 1
                ...
                if "part2" in key:
                    if file_format == "Amp_Phase":
                        ...
                        if "u" in self.ncglo[key] or "v" in self.ncglo[key]:
                            print(
                                "looking at velocity Imaginary part. Assume it is in meter.second-1"
                            )
                            uni = "meter.second-1"
                        else:
                    ...
                    if file_format == "Amp_Phase":
                        if "u" in self.ncglo[key] or "v" in self.ncglo[key]:
                            print(
                                "looking at a velocity amplitude. Assume it is in meter.second-1"
                            )
                            uni = "meter.second-1"
                        ...
                        if "u" in self.ncglo[key] or "v" in self.ncglo[key]:
                            print(
                                "looking at velocity Real part. Assume it is in meter.second-1"
                            )
                            uni = "meter.second-1"
                        else:
        ...
        [
            ...
            self.jdmax,
            self.period,
        ] = self.handle_periodicity(crocogrd, dataxr_ssh, "r")
        ...
        if currentu is not None:
            [
                ...
                self.jdmaxU,
                self.periodU,
            ] = self.handle_periodicity(crocogrd, dataxr_u, "u")
            [
                self.lonV,
                self.latV,
                self.idminV,
                ...
                self.jdmaxV,
                self.periodV,
            ] = self.handle_periodicity(crocogrd, dataxr_v, "v")
        ...
        for key in self.ncglo:
            if "u" in self.ncglo[key]:
                ...
                if (
                    ...
                    self.ncglo[key][:].shape[1] == nx
                    and self.ncglo[key][:].shape[2] == ny
                ):
    ...
    def ap2ep(self, Au, PHIu, Av, PHIv):
        ...
        id = THETAp < 0
        THETAp[id] = THETAp[id] + 360
        id = THETAm < 0
        THETAm[id] = THETAm[id] + 360
        id = PHA < 0
        PHA[id] = PHA[id] + 360
        id = INC < 0
        INC[id] = INC[id] + 360
    ...
    def handle_periodicity(self, crocogrd, data, grid):
        ...
        geolim = [
            crocogrd.lonmin() - buffer_zone,
            crocogrd.lonmax() + buffer_zone,
            crocogrd.latmin() - buffer_zone,
            crocogrd.latmax() + buffer_zone,
        ]

        jmin = self.indx_bound(lat.data, geolim[2])
        jmax = self.indx_bound(lat.data, geolim[-1])

        if -1 < jmin and jmin < lat.shape[0] and -1 < jmax and jmax < lat.shape[0]:
            if jmin > 0:
                jmin = jmin - 1
            jmax = jmax + 2
        else:
            print(
                "North-south extents of the dataset ",
                lat.data[0],
                lat.data[-1],
                " are not sufficient to cover the entire model grid.",
            )
            sys.exit()
        ####
        ...
        if -1 < imin and imin < lon.shape[0] and -1 < imax and imax < lon.shape[0]:
            if imin > 0:
                imin = imin - 1
            imax = imax + 1
            shft_west = 0
            ...
            if period > 0:
                print(
                    "Identified periodicity domain in data of ",
                    period,
                    " points out of",
                    lon.shape[0],
                )
                ...
                print(
                    "ERROR: The data does not cover the entire grid. Change your grid definition"
                )
                sys.exit()
            ##
            shft_west = 0
            ...
            if -1 < imin and imin < lon.shape[0] and -1 < imax and imax < lon.shape[0]:
                ...
                print(
                    "ERROR: Data longitude covers 360 degrees, but still cannot find  starting and ending indices."
                )
                sys.exit()
        ...
        ###
            ...
            print(
                "Reading topography in two separate parts adjacent through 360-degree periodicity\n First..."
            )
    ...
    def var_periodicity(self, vname, l, k, bdy=""):
        ...
        if imin < imax:
            ...
            if k == -1:
                field = np.array(
                    np.squeeze(
                        self.ncglo[vname][mintime:maxtime, start2:end2, start1:end1]
                    )
                )
            else:
                field = np.array(
                    np.squeeze(
                        self.ncglo[vname][mintime:maxtime, k, start2:end2, start1:end1]
                    )
                )
    ...
    def egbert_correction(self, input_wav, date_ini):
        ...
        timetemp = (
            cftime.date2num(date_ini, "days since 1858-11-17:00:00:00") - 51544.4993
        )
        ...
        if input_wav.lower() in [
            ...
            pf = np.sqrt((1.0 + 0.188 * cosn) ** 2 + (0.188 * sinn) ** 2)
            ...
            pf = np.sqrt(
                (one + 0.203 * cosn + 0.040 * cos2n) ** 2
                + (0.203 * sinn + 0.040 * sin2n) ** 2
            )
        elif input_wav.lower() == "chi1":
            pf = np.sqrt((1.0 + 0.221 * cosn) ** 2 + (0.221 * sinn) ** 2)
            ...
            pf = np.sqrt((1.0 + 0.169 * cosn) ** 2 + (0.227 * sinn) ** 2)
            ...
            pf = np.sqrt((1.0 + 0.436 * cosn) ** 2 + (0.436 * sinn) ** 2)
        ...
        if input_wav.lower() in [
            ...
            pu = np.arctan(0.189 * sinn / (1.0 + 0.189 * cosn))
            ...
            pu = np.arctan(-0.436 * sinn / (1.0 + 0.436 * cosn))
        elif input_wav.lower() == "j1":
            pu = np.arctan(-0.227 * sinn / (1.0 + 0.169 * cosn))
        elif input_wav.lower() == "oo1":
            pu = np.arctan(
                (-0.03731 * sinn + 0.00052 * sin2n)
                / (1.0 - 0.03731 * cosn + 0.00052 * cos2n)
            ...
            pu = np.arctan(-0.221 * sinn / (1.0 + 0.221 * cosn))
        elif input_wav.lower() == "m1":
            ...
            pu = np.arctan(
                -(0.203 * sinn + 0.040 * sin2n) / (one + 0.203 * cosn + 0.040 * cos2n)
            )
        else:
