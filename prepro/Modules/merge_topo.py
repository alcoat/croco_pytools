...
def create_depth_grid_from_points(
    data_file_path,
    shapefile_path,
    lat_bounds,
    lon_bounds,
    ...
    df = pd.read_csv(
        data_file_path, sep="\s+", header=None, names=["lat", "lon", "depth"]
    )
    ...
    try:
        ...
        transformer = Transformer.from_crs(
            f"EPSG:{epsg_int}", shapefile_crs, always_xy=True
        )
        ...
        print(
            "Assuming the shoreline file is in the same reference system as the bathymetry..."
        )
    ...
    if gdf.crs.to_epsg() != epsg_int:
        ...
        gdf = gdf.to_crs(
            epsg=epsg_int
        )  # Reproject the shapefile to match text data EPSG
    ...
    grid_lon, grid_lat = np.mgrid[
        lon_bounds[0] : lon_bounds[1] : num_points_lon * 1j,
    ...
    mask = np.zeros_like(
        grid_lon, dtype=bool
    )  # Initialize the mask as a boolean array with the same shape as grid_lon
    ...
    for i, j in product(list(range(nchunk)), list(range(nchunk))):
        ...
        dy2 = 2
        if i == 0:
            dx1 = 0  # No overlap on the left-most chunk
        if i == nchunk - 1:
            dx2 = 0  # No overlap on the right-most chunk
        if j == 0:
            dy1 = 0  # No overlap on the top-most chunk
        if j == nchunk - 1:
            dy2 = 0  # No overlap on the bottom-most chunk
        ...
        llcrnrlon = np.nanmin(
            grid_lon[ny1i:ny2i, nx1i:nx2i]
        )  # Lower-left corner longitude
        urcrnrlon = np.nanmax(
            grid_lon[ny1i:ny2i, nx1i:nx2i]
        )  # Upper-right corner longitude
        llcrnrlat = np.nanmin(
            grid_lat[ny1i:ny2i, nx1i:nx2i]
        ...
        try:  # Try to apply region masking if polygons exist in the chunk
            ...
            mask[ny1i + dy1 : ny2i - dy2, nx1i + dx1 : nx2i - dx2][
                np.isnan(rmask[dy1 : ny2i - ny1i - dy2, dx1 : nx2i - nx1i - dx2])
            ] = 1
    ...
    ds = xr.Dataset(
        ...
        coords={"lon": (["lon"], grid_lon[:, 0]), "lat": (["lat"], grid_lat[0, :])},
    )
...
def interpolate_large_grid(
    ...
    grid_interpolator = pyinterp.backends.xarray.RegularGridInterpolator(
        data_array, geodetic=False
    )

    grid_shape = longitudes.shape  # Get the shape of the grid to interpolate

    interpolated_result = np.full(
        grid_shape, np.nan
    )  # Initialize the result grid with NaN
    ...
    if grid_shape[0] * grid_shape[1] > size_threshold:
        ...
        for row_start in range(0, grid_shape[0], chunk_size - overlap):
            for col_start in range(0, grid_shape[1], chunk_size - overlap):
                ...
                lon_chunk = longitudes[
                    row_start_overlap:row_end, col_start_overlap:col_end
                ].ravel()  # .flatten()
                lat_chunk = latitudes[
                    row_start_overlap:row_end, col_start_overlap:col_end
                ].ravel()  # .flatten()
                ...
                coords_chunk = {"longitude": lon_chunk, "latitude": lat_chunk}

                # Perform interpolation for this chunk
                interpolated_chunk = grid_interpolator(
                    coords_chunk, method=method
                ).reshape(row_end - row_start_overlap, col_end - col_start_overlap)
        ...
        interpolated_result = grid_interpolator(flat_coords, method=method).reshape(
            longitudes.shape
        )
...
def find_coords(ds):
    ...
    for coord_name in ds.coords:
        coord_lower = (
            coord_name.lower()
        )  # Convert the coordinate name to lowercase for comparison
    ...
    if lon_coord is None or lat_coord is None:
        raise ValueError(
            "❌ Could not find valid longitude/latitude coordinates in the dataset."
        )
...
def merge_multiple_grids(grid_files, buffer_width, final_output_file, coarsen_factors):
    ...
    for i in range(1, len(grid_files)):
        ...
        print(f"\n=== Iteration {i} ===")
        print(
            f"Merging grids: High resolution: {current_high_res}, Low resolution: {low_res}"
        )
        ...
        merge_smooth(
            ...
            buffer_width,
            temp_output_file,
            coarsen_factor=coarsen_factors[i - 1],
        )
    ...
    merge_smooth(
        ...
        buffer_width,
        final_output_file,
        coarsen_factor=coarsen_factors[-1],
    )
...
def merge_smooth(
    ...
    size_threshold = (
        3000000  # Maximum grid size (number of elements) for direct processing
    )
    ...
    if proj_high_res and proj_low_res:
        if proj_high_res != proj_low_res:
            print(
                f"⚠️ The datasets are in different projections. Checking for necessary reprojections to {target_epsg}..."
            )
            ...
            if proj_high_res != target_crs:
                ...
                transformer_high_to_target = Transformer.from_crs(
                    proj_high_res, target_crs, always_xy=True
                )
                high_res_coords = transformer_high_to_target.transform(
                    ds1["lon"].values, ds1["lat"].values
                )
                ds1["lon"], ds1["lat"] = high_res_coords
            else:
                print(
                    f"✅ High resolution dataset is already in {target_epsg}, no reprojection needed."
                )
            ...
            if proj_low_res != target_crs:
                ...
                transformer_low_to_target = Transformer.from_crs(
                    proj_low_res, target_crs, always_xy=True
                )
                low_res_coords = transformer_low_to_target.transform(
                    ds2["lon"].values, ds2["lat"].values
                )
                ds2["lon"], ds2["lat"] = low_res_coords
            else:
                print(
                    f"✅ Low resolution dataset is already in {target_epsg}, no reprojection needed."
                )
    ...
    # Handle cases where one or both datasets lack a CRS
        ...
        if proj_high_res is None and proj_low_res is None:
            print(
                "⚠️ Warning: Both high and low resolution datasets lack a defined CRS."
            )
            ...
            print(
                f"➡️ Assuming it matches the low resolution dataset CRS and is in {target_epsg}."
            )
        elif proj_low_res is None:
            print("⚠️ Warning: Low resolution dataset lacks a defined CRS.")
            print(
                f"➡️ Assuming it matches the high resolution dataset CRS and is in {target_epsg}."
            )
    ...
    for var_name in ds1.data_vars:
        ...
        raise ValueError(
            "❌ No valid bathymetry variable found in the high resolution dataset (ds1)"
        )
    ...
    for var_name in ds2.data_vars:
        ...
        raise ValueError(
            "❌ No valid bathymetry variable found in the low resolution dataset (ds2)"
        )
    ...
    if coarsen_factor:
        ds1 = ds1.coarsen(
            {lat_coord_ds1: coarsen_factor, lon_coord_ds1: coarsen_factor},
            boundary="trim",
        ).mean()
        print(
            "✅ Coarsening completed. The high-resolution grid has been successfully downsampled."
        )
    ...
    if downscale_bounds:
        if len(downscale_bounds) != 4:
            raise ValueError(
                "❌ downscale_bounds must be a list of four elements [lon_min, lon_max, lat_min, lat_max]"
            )

        lon_min, lon_max, lat_min, lat_max = downscale_bounds
        if lon_min >= lon_max or lat_min >= lat_max:
            raise ValueError(
                "❌ Longitude/Latitude bounds are not valid: ensure lon_min < lon_max and lat_min < lat_max"
            )
        ...
        print(
            "✅ Downscaling completed. The low-resolution grid has been successfully cropped to the specified bounds."
        )
    ...
    """
    z2_interp = interpolate_large_grid(
        new_lat_grid_2,
        new_lon_grid_2,
        z2_dataarray,
    ...
    mask_overlap = (
        (new_lon_grid_2 >= lon_min_1)
        & (new_lon_grid_2 <= lon_max_1)
        & (new_lat_grid_2 >= lat_min_1)
        & (new_lat_grid_2 <= lat_max_1)
    )
    ...
    z1_interp = interpolate_large_grid(
        new_lat_grid_2,
        new_lon_grid_2,
        z1_dataarray,
        chunk_size=chunk_size,
    ...
    def nan_buffer_linear_pond(blended_data, low_res_interp, buffer_width):
        ...
        new_z[mask_eroded] = low_res_interp[
            mask_eroded
        ]  # Low-res data in the eroded zone

        # Get coordinates of transition and transition border areas
        coords_mask_transition = np.array(np.where(mask_transition)).T
        coords_transition_borders_int = np.array(
            np.where(transition_borders & mask_eroded)
        ).T
        coords_transition_borders_ext = np.array(
            np.where(transition_borders & ~mask_eroded)
        ).T
        ...
        distances_int, indices_int = tree_transition_borders_int.query(
            coords_mask_transition
        )
        distances_ext, indices_ext = tree_transition_borders_ext.query(
            coords_mask_transition
        )
        ...
        def apply_ponderated_values(
            new_z, mask_transition, ponderated_values, coords_mask_transition
        ):
            # Apply weighted values to z2_combined based on transition mask
            for i, (coord, value) in enumerate(
                zip(coords_mask_transition, ponderated_values)
            ):
                x, y = coord
        ...
        new_z = apply_ponderated_values(
            new_z, mask_transition, ponderated_transition, coords_mask_transition
        )
    ...
    for width in buffer_widths:
        print(
            f"⚙️ Filling NaN areas with lower-resolution data and smoothing the buffer of {width} cells..."
        )
        ...
        print(
            f"✅ Successfully created a {width}-cell buffer, enhancing data quality around NaN areas."
        )
        # ---> Clean memory:
        gc.collect()

    print(
        "🎉 Groups of NaNs in the high-resolution grid have been filled with lower-resolution grid data and smoothed."
    )
    ...
    if new_lon_2.size > size_threshold:
        print(
            "⚙️ The grid is large; using chunk processing with overlap to fill last ponctual Nans..."
        )
        ...
        for i in range(0, z_less_nan.shape[0], chunk_size - overlap):
            for j in range(0, z_less_nan.shape[1], chunk_size - overlap):
                ...
                lat_chunk_axis = pyinterp.Axis(
                    lat_axis[i:i_end].astype(np.float64)
                )  # lat_axis[i:i_end]
                lon_chunk_axis = pyinterp.Axis(
                    lon_axis[j:j_end].astype(np.float64)
                )  # lon_axis[j:j_end]
        ...
        print(
            "⚙️ The grid is small; applying LOESS interpolation directly to fill the last ponctual Nans..."
        )
    ...
    def create_buffer_on_side(
        start_idx, end_idx, buffer_limit, buffer_width, max_limit, side
    ):
        if buffer_limit <= min_buffer_width:  # Not enough space for a proper buffer
            print(
                f"⚠️ Warning: Not enough space for a buffer on the {side} side. No smoothing applied."
            )
            return 0
        elif buffer_limit < buffer_width:  # Reduce buffer size
            print(
                f"⚠️ Warning: The buffer on the {side} side has been reduced to {buffer_limit} cells."
            )
    ...
    buffer_top = create_buffer_on_side(
        top, bottom, top, buffer_width, grid_shape[0], "top"
    )
    if buffer_top > 0 and has_valid_data_outside(
        top - buffer_top, top, left, right + 1
    ):
        mask_buffer[top - buffer_top : top, left : right + 1] = True

    # If the bottom of the mask_overlap is not at the bottom edge of the grid
    buffer_bottom = create_buffer_on_side(
        bottom,
        bottom + buffer_width,
        grid_shape[0] - bottom - 1,
        buffer_width,
        grid_shape[0],
        "bottom",
    )
    if buffer_bottom > 0 and has_valid_data_outside(
        bottom + 1, bottom + 1 + buffer_bottom, left, right + 1
    ):
    ...
    buffer_left = create_buffer_on_side(
        left, right, left, buffer_width, grid_shape[1], "left"
    )
    if buffer_left > 0 and has_valid_data_outside(
        top, bottom + 1, left - buffer_left, left
    ):
        mask_buffer[top : bottom + 1, left - buffer_left : left] = True

    # If the right of the mask_overlap is not at the right edge of the grid
    buffer_right = create_buffer_on_side(
        right,
        right + buffer_width,
        grid_shape[1] - right - 1,
        buffer_width,
        grid_shape[1],
        "right",
    )
    if buffer_right > 0 and has_valid_data_outside(
        top, bottom + 1, right + 1, right + 1 + buffer_right
    ):
    ...
    def include_corners(mask_buffer, top, bottom, left, right, buffer_width):
        ...
        if top - buffer_width >= 0 and right + buffer_width < grid_shape[1]:
            mask_buffer[
                top - buffer_width : top, right + 1 : right + 1 + buffer_width
            ] = True
        # Bottom-left corner
        if bottom + buffer_width < grid_shape[0] and left - buffer_width >= 0:
            mask_buffer[
                bottom + 1 : bottom + 1 + buffer_width, left - buffer_width : left
            ] = True
        # Bottom-right corner
        if (
            bottom + buffer_width < grid_shape[0]
            and right + buffer_width < grid_shape[1]
        ):
            mask_buffer[
                bottom + 1 : bottom + 1 + buffer_width,
                right + 1 : right + 1 + buffer_width,
            ] = True
    ...
    for i, coord in enumerate(coords_buffer):
        ...
        if total_distance == 0:
            ...
            interpolated_values_buffer[i] = (
                weight_inner * value_inner + weight_outer * value_outer
            )
    ...
    try:
        ...
        topo_type = {"lon": "lon", "lat": "lat", "topo": "topo", "zaxis": "up"}
    ...
    ds_interpolated = xr.DataArray(
        z.astype("float32"),  # Cast to float32
        coords={
            topo_type["lat"]: new_lat_unique,
            topo_type["lon"]: new_lon_unique,
    ...
    # CRS attribute for DataArray
    epsg_int = int(target_epsg.split(":")[1])
    crs = CRS.from_epsg(epsg_int)  # EPSG:4326 for WGS84
    crs_proj4 = crs.to_proj4()

    # Ajouter l'attribut CRS à l'objet DataArray
    ds_to_save.attrs["crs"] = crs_proj4

    # SAVE WITH CHUNKS TO LIMIT MEMORY USAGE
    ds_to_save.to_netcdf(
        output_file,
        encoding={topo_type["topo"]: {"chunksizes": (100, 100), "dtype": "float32"}},
    )
...
def kring_merging(
    ...
    try:
        # Essayer avec un séparateur d'espace
        df = pd.read_csv(
            high_res, sep="\s+", header=None, names=["lat", "lon", "depth"]
        )
        print("🎯 Fichier lu avec succès (séparateur espace)")
    except pd.errors.ParserError:
        # Si une erreur de parsing survient, essayer avec une virgule comme séparateur
        df = pd.read_csv(high_res, sep=",", header=None, names=["lat", "lon", "depth"])
        print("🎯 Fichier lu avec succès (séparateur virgule)")

    # Force coversion to flaot type
    ...
    try:
        ...
        min_lon, min_lat = lat_bounds
        max_lon, max_lat = lon_bounds
    ...
    try:
        ...
        # y=slice(lat_bounds[0], lat_bounds[1]), x=slice(lon_bounds[0], lon_bounds[1])
        # )
        subset = ds_netcdf.where(
            (ds_netcdf["y"] >= lat_bounds[0])
            & (ds_netcdf["y"] <= lat_bounds[1])
            & (ds_netcdf["x"] >= lon_bounds[0])
            & (ds_netcdf["x"] <= lon_bounds[1]),
            drop=False,  # Cela permet de ne pas supprimer les dimensions vides
        )
        ...
        df_subset = pd.DataFrame({"x": X_flat, "y": Y_flat, "depth": depth_flat})
    ...
    for i, j in product(list(range(nchunk)), list(range(nchunk))):
        ...
        # Find the geographic boundaries of the current chunk in lon/lat
        llcrnrlon = np.nanmin(X[ny1i:ny2i, nx1i:nx2i])  # Lower-left corner longitude
        urcrnrlon = np.nanmax(X[ny1i:ny2i, nx1i:nx2i])  # Upper-right corner longitude
        llcrnrlat = np.nanmin(Y[ny1i:ny2i, nx1i:nx2i])  # Lower-left corner latitude
        urcrnrlat = np.nanmax(Y[ny1i:ny2i, nx1i:nx2i])  # Upper-right corner latitude
        ...
        try:  # Try to apply region masking if polygons exist in the chunk
            ...
            rmask = regionmask.mask_geopandas(
                gs.geometry,
                X[ny1i:ny2i, nx1i:nx2i],
                Y[ny1i:ny2i, nx1i:nx2i],
            )
    ...
    ds = xr.Dataset(
        {"depth": (["lon", "lat"], np.transpose(grid_depth_masked))},
        coords={"lon": (["lon"], X[0, :]), "lat": (["lat"], Y[:, 0])},
    )
...
def mblend_smoothing(high_res, low_res, far_edge, output_file, gisbase, EPSG_num):
    ...
    try:
        ...
        print(
            f"Required module is missing: {e}. Please install GRASS GIS and related dependencies."
        )

    # Configuration paths for GRASS GIS
    gisdbase = (
        tempfile.mkdtemp()
    )  # Create a temporary directory for the GRASS GIS database
    ...
    def reprojeter_raster(input_file, output_file):
        ...
        try:
            subprocess.run(
                ["gdalwarp", "-t_srs", "EPSG:4326", input_file, output_file], check=True
            )
    ...
    location = binascii.hexlify(
        os.urandom(16)
    ).decode()  # Generate a random name for the location
    mapset = "PERMANENT"  # Default mapset name
    location_path = os.path.join(gisdbase, location)  # Path to the new location
    startcmd = (
        f"grass78 -c epsg:4326 -e {location_path}"  # Command to create the location
    )

    print(f"Creating location with command: {startcmd}")
    subprocess.run(
        startcmd, shell=True, check=True
    )  # Execute the command to create the location
    ...
    gscript.run_command("r.mblend", high=raster1, low=raster2, output=output)
