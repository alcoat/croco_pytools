...
if [[ ${READ_GRD} == 1 ]]; then
    ...
    rm -f tmp.nc
    lon_min=`echo "scale=4; ${lon_min} - 1.5" | bc `
    lat_min=`echo "scale=4; ${lat_min} - 1.5" | bc `
    lon_max=`echo "scale=4; ${lon_max} + 1.5" | bc `
    lat_max=`echo "scale=4; ${lat_max} + 1.5" | bc `
...
##
        if [[ ${kdata} == "MONTHLY" ]]; then
			...
			OUTNAME="${OUTDIR}/raw_motu_${PREFIX}_Y${YEAR}M$(printf "%02d" ${MONTH}).nc"            
